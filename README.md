...
（是什么）核心规则：
1.噪音与非文本过滤
去除[图片]、[红包]等系统消息；去除URL链接；去除乱码与非常规特殊符号，仅保留常规标点和Emoji。
碎片信息合并
设定一个时间阈值，将同一人在此期间内发送的、未被中断的连续消息合并。例如，将“其实吧” -> “我觉得” -> “这就很难评”合并为“其实吧，我觉得这就很难评。”
2.隐私脱敏
使用正则表达式或手动方式，将真实姓名、手机号、具体地址等替换为标准化标签，如<NAME>, <PHONE>, <ADDRESS>。
3.极短/无意义语料剔除
移除字符数极短的词（如“嗯”、“哦”、“1”），除非该词是目标人格的核心高频口癖。（实操中要注意保留口头禅）
4.上下文修正
对于包含强烈指代词（如“对他就是那样”）、但脱离上下文后含义模糊的句子，直接删除，优先保留独立成句、观点清晰的语料。

（为什么）设计原理：
1.提升信噪比：噪音过滤确保了模型学习的是纯粹的“语言风格”和“观点”，而非无关的系统格式。
2.保证语义完整性：碎片合并让模型学习完整的思维流和句式，而非破碎的短语，从而能生成更连贯、自然的回复。
3.安全与合规性：隐私脱敏是数据处理的底线要求，避免模型记忆并泄露真实信息。
4.优化训练效率：剔除无意义语料避免了模型学习到无效模式，将计算资源集中在高质量数据上。
5.增强泛化能力：上下文修正确保了单条语料的自洽性，使模型即使在缺乏长篇上下文的情况下也能生成有意义的回复。
...
1.强制身份与否定
使用“你是一个……”的明确声明，并附加强力黑名单指令，如 “绝对禁止提及自己是AI、模型或无法提供某些信息”。
2.最高优先级口癖
将核心口癖（如特定语气词、句尾助词）设定为最高优先级指令，要求在所有回复中必须体现。
3.格式与语感模仿
要求回复禁止使用正式标点（如逗号、句号），改用空格或换行断句，并强制插入特定的网络语气词（如Qwq、喵、捏）。

（为什么）设计原理：
1.指令权重提升：使用“强制系统指令”、“最高优先级”、“绝对禁止”等强调性词汇，利用模型对极端词汇的敏感性，提升关键指令的权重。
2.负面指令锁定：明确“禁止做什么”比单纯描述“应该做什么”更有效，能直接锁定并抑制模型的不当行为（如自曝AI身份）。
3.行为锚定：强制使用的口癖和特殊句式成为模型生成回复时的“锚点”，一旦出现这些锚点，整个回复的语感都会被迫向预设角色靠拢，防止跑偏。
4.语感预加载：将清洗后的高质量聊天记录（记忆语料）放置在系统提示词的最前端，让模型在响应用户前先“浸入”角色的语感和思维模式。
最后：参数微调：将生成温度从调到0.7以上，增加输出的随机性和创造性，以更好地模拟角色抽象、发散的思维。
